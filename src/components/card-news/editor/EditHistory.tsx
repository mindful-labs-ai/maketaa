...
/**
 * EditHistory Component - Shows edit history for a card spec
 */
...
import {
  ...
  formatEditValue,
  groupEditsByCard,
} from '@/lib/card-news/editLogger';
import type { EditLogEntry } from '@/lib/card-news/editLogger';
...
export function EditHistory({
  ...
  useEffect(() => {
    const loadHistory = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const history = await getEditHistory({ specId, limit: 100, offset: 0 });
        setEdits(history);
      } catch (err) {
  ...
  const handleShowMore = () => setDisplayCount((prev) => prev + itemsPerPage);
