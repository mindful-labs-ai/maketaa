...
import { LAYOUT_TYPES, BACKGROUND_TYPES, COLOR_PALETTES, CANVAS_RATIOS, FONT_OPTIONS, TEXT_LIMITS, type CanvasRatio } from '@/lib/constants';
import { ChevronDown, Upload, X } from 'lucide-react';
import { searchPhotos, hasUnsplashKey, suggestQuery, type UnsplashPhoto } from '@/lib/unsplash';
...
export const StylePanel = React.forwardRef<HTMLDivElement, StylePanelProps>(
  ({ className = '' }, ref) => {
    ...
    // Local text state — kept in sync with store via useEffect for real-time canvas edits
    ...
    // Sync local text from store whenever the selected card changes or store updates
    useEffect(() => {
      if (cardText) {
        setLocalText({
    ...
    const handleContentInput = (
      ...
      if (contentDebounceRef.current[field]) {
        clearTimeout(contentDebounceRef.current[field]);
      }
    ...
    const handleAddField = (
      field: 'body' | 'sub_text' | 'description' | 'quote'
    ) => {
    ...
    const handleBulletInput = (index: number, value: string) => {
      ...
      if (bulletDebounceRef.current[index]) {
        clearTimeout(bulletDebounceRef.current[index]);
      }
    ...
    // When the selected card changes, reset search and pre-fill suggested query
    useEffect(() => {
      if (selectedCard) {
    ...
    const handleFontFamilyChange = async (
      fontType: 'headline_family' | 'body_family',
      value: string
    ) => {
      if (isDisabled) return;
      try {
    ...
    return (
      <div
        ref={ref}
        className={`
          w-64 bg-white border-l border-gray-200 overflow-y-auto flex flex-col scrollbar-thin
          ${className}
        `}
      >
        {/* Header */}
        ...
        <div className={isDisabled ? 'opacity-40 pointer-events-none' : ''}>
              ...
              <div className="space-y-3">
                ...
                {cardText.body !== undefined ? (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleRemoveField('body')}
                  ...
                  <button
                    ...
                    className="w-full py-1.5 border border-dashed border-gray-300 rounded-md text-[11px] text-gray-400 hover:border-gray-500 hover:text-gray-600 transition-colors"
                  >
                    + 본문 추가
                  </button>
                )}
                ...
                {cardText.sub_text !== undefined ? (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleRemoveField('sub_text')}
                          className="text-gray-300 hover:text-gray-500 transition-colors"
                          title="부제 제거"
                        >
                          <span className="text-[12px] leading-none">&times;</span>
                  ...
                  <button
                    onClick={() => handleAddField('sub_text')}
                    className="w-full py-1.5 border border-dashed border-gray-300 rounded-md text-[11px] text-gray-400 hover:border-gray-500 hover:text-gray-600 transition-colors"
                  >
                    + 부제 추가
                  </button>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleRemoveField('quote')}
                          className="text-gray-300 hover:text-gray-500 transition-colors"
                          title="인용문 제거"
                        >
                          <span className="text-[12px] leading-none">&times;</span>
                ...
                {/* Bullet Points */}
                <div>
                  <label className="block text-[10px] text-gray-400 uppercase tracking-wide mb-1.5">
                    목록 항목
                  </label>
          ...
          {/* Background Section — moved before palette so users set type first */}
          <CollapsibleSection
            title="배경"
              ...
              <div className="space-y-3">
                ...
                {background.type === 'image' && (
                  <div className="space-y-3">
                      ...
                      <div className="relative w-full h-20 rounded-lg border border-gray-200 overflow-hidden group">
                        ...
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="px-2 py-1 bg-white text-gray-800 rounded text-[11px] font-medium hover:bg-gray-100"
                          >
                            변경
                          </button>
                          <button
                            onClick={handleClearUpload}
                            className="p-1 bg-white text-gray-800 rounded hover:bg-gray-100"
                          >
                            <X size={11} />
                          </button>
                        </div>
                    ...
                    {/* Current background thumbnail (from Unsplash or existing src) */}
                            ...
                            <div className="grid grid-cols-3 gap-1">
                              {searchResults.map((photo) => (
                                <button
                                  ...
                                  style={{
                                    borderColor: selectedPhotoId === photo.id ? '#111827' : 'transparent',
                                  }}
                            ...
                            {selectedPhotoId && (() => {
                              ...
                              return photo ? (
                                <p className="text-[10px] text-gray-400 mt-1.5">
                                  Photo by{' '}
                                  <a
                                    href={`${photo.user.links.html}?utm_source=card_news_maker&utm_medium=referral`}
                                    target="_blank"
                                  ...
                                  on{' '}
                                  <a
                                    href="https://unsplash.com?utm_source=card_news_maker&utm_medium=referral"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="underline hover:text-gray-600"
                                  >
          ...
          >
            <div>
              ...
              {/* Compact preview bar */}
              <div className="mt-3 h-6 rounded-md overflow-hidden border border-gray-200">
                <div
